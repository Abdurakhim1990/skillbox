...
bool PlayerAttack(int play[][10])
{
    int coord_x, coord_y;

    do{
        cin >> coord_x >> coord_y;
        if(coord_x >= 0 && coord_x <= 9 && coord_y >= 0 && coord_y <= 9) break;
        cout << "Wrong coordinates!!!" << endl;
    } while(1);
    if(play[coord_x][coord_y] == 1){
        play[coord_x][coord_y] = -1;
        cout << "Hit!" << endl;
        return true;
    } else{
        play[coord_x][coord_y] = -2;
        cout << "Past!" << endl;
        return false;
    }
}
...
int main()
{
    ...
    int count_1 = 20, count_2 = 20;
    while (count_1 > 0 && count_2 > 0) {
        cout << "Enter the coordinates of the attack of player 1: ";
        count_2 -= PlayerAttack(player_2);
        OutputFields(player_1, player_2);
        cout << "Enter the coordinates of the attack of player 2: ";
        count_1 -= PlayerAttack(player_1);
        OutputFields(player_1, player_2);
    }
    if(count_2 == 0){
        cout << "Won by player 1." << endl;
    } else{
        cout << "Won by player 2." << endl;
    }
    return 0;
}
